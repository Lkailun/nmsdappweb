...
const Button: FC<ButtonProps> = ({ children, onClick, style, id, className, loading, disabled }: ButtonProps): ReactElement => {
    return (
        // <ButtonView id={id} className={cn(styles.btn, className, loading && styles.loading, disabled && styles.disabled)} style={style} onClick={onClick}>
        <ButtonView
            id={id}
            className={cn(className, loading && 'loading', disabled && 'disabled')}
            style={style}
            disabled={disabled}
            onClick={() => {
